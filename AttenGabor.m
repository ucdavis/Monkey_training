...
try
    ...
    % Set pathway
    path = strcat('D:\EyelinkData\','AtttenGabor\',cfg.sub,'\',currDate,'\'); % where to keep the edf files
    ...
    red_color = [255, 0, 0, 800];
    green_color=[0, 255, 0, 800];
    ...
    elseif cfg.polar
        % polar
        v_x_sp=cfg.radius*ppd*cos(deg2rad(cfg.degree));
        v_y_sp=cfg.radius*ppd*sin(deg2rad(cfg.degree));
    end
    ...
    skip_trial=0;
    Num_tried=0;
    gabor_list= repmat([ones(10,1);2*ones(10,1)],500,1);
    ...
    while trial_success>0
        switch(stage)
            case 'trial_new_start'
                ...
                gabor_ori=gabor_list(trial_success+skip_trial);%randi([1, 2]);
                %gabor_ori=1;
                Results.gabor_ori(trial_total)=gabor_ori;
            ...
            case 'put_on_gabor'
                ...
                gaborSize=round(cfg.gaborSize*ppd);
                gaborTex = CreateProceduralGabor(window, gaborSize, gaborSize, [], [0.5 0.5 0.5 1], disableNorm, preContrastMultiplier);
                if gabor_ori==1
                    orientation=180;
                elseif gabor_ori==2
                    orientation=90;
                ...
                % draw fixation point, update time
%                 Screen('FillRect',window, el.backgroundcolour);
                xVertices = [center(1), center(1) - 10, center(1) + 10];
                yVertices = [center(2) - 10, center(2) + 10, center(2) + 10];
                phase = 0;
                numCycles = 4;
                freq = numCycles / gaborSize;
                sigma = gaborSize / 7;
                contrast = 1;
                aspectRatio = 1.0;
                ...
                GaborRect= [center(1)-gaborSize/2+x_sp, center(2)-gaborSize/2-y_sp, center(1)+gaborSize/2+x_sp, center(2)+gaborSize/2-y_sp];
                % Draw the filled triangle

%                 Screen('FillPoly', window, fp_color, [xVertices; yVertices]', 1);
                Screen('FillOval',window,[0 0 255 1000], [center(1)-fpr+x_fp, center(2)-fpr-y_fp, center(1)+fpr+x_fp, center(2)+fpr-y_fp],5);
               Screen('FillOval',window,el.backgroundcolour, [center(1)-fpr/1.1+x_fp, center(2)-fpr/1.1-y_fp, center(1)+fpr/1.1+x_fp, center(2)+fpr/1.1-y_fp],5);

                Screen('DrawTextures', window, gaborTex, [], GaborRect, orientation, [], [], [], [], kPsychDontDoRotation, propertiesMat');
                ...
                checkkeys;
                if gabor_ori==1
                    stage='wait_fix_gabor';
                elseif gabor_ori==2 || gabor_ori==3
                    stage='keep_hold_fp';
                end
            case 'keep_hold_fp'
            ...
            case 'wait_for_hold_gabor'
                ...
                if not((x_eye >= boxsp1)&&(x_eye <= boxsp3)&&(y_eye >= boxsp2)&&(y_eye <= boxsp4))
                    ...
                    stage='inter_trial_interval';
                elseif GetSecs-t_checkpoint>=cfg.t_fixation_sp/2
                    % eyelink, matlab message, hold comp fp
            ...
            case 'error_indication'
                if gabor_ori==1
                    Screen('FillOval',window,red_color, [center(1)-fpr*4+x_fp, center(2)-fpr*4-y_fp, center(1)+fpr*4+x_fp, center(2)+fpr*4-y_fp],5);
                    t_checkpoint=Screen('Flip',window);
                elseif gabor_ori==2 ||gabor_ori==3
                    Screen('FillOval',window,red_color, [center(1)-spr*4+x_sp, center(2)-spr*4-y_sp, center(1)+spr*4+x_sp, center(2)+spr*4-y_sp],5);
                    t_checkpoint=Screen('Flip',window);

                end
                WaitSecs(cfg.WaitTime)
                stage='inter_trial_interval';
            case 'reward_indication'
                if gabor_ori==1
                    Screen('FillOval',window,green_color, [center(1)-spr*4+x_sp, center(2)-spr*4-y_sp, center(1)+spr*4+x_sp, center(2)+spr*4-y_sp],5);
                    t_checkpoint=Screen('Flip',window);
                elseif gabor_ori==2 || gabor_ori==3
                    Screen('FillOval',window,green_color, [center(1)-fpr*4+x_fp, center(2)-fpr*4-y_fp, center(1)+fpr*4+x_fp, center(2)+fpr*4-y_fp],5);
                    t_checkpoint=Screen('Flip',window);
