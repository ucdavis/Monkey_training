...
try
    ...
    spr=round(ppd*cfg.spr);
    v_x_fp=cfg.fp_x*ppd; % x position of the fixation points , round(-8*ppd), round(8*ppd)
    v_y_fp=cfg.fp_y*ppd;
    if ~cfg.polar
        % y position of the fixation points , round(-8*ppd), round(8*ppd)
        v_x_sp=cfg.sp_x*ppd;
        v_y_sp=cfg.sp_y*ppd;
    elseif cfg.polar
        v_x_sp=cfg.radius*ppd*cos(cfg.degree); % x position of the fixation points , round(-8*ppd), round(8*ppd)
    ...
    if ~cfg.polar
        ...
        [A_sp,B_sp] = meshgrid(v_x_sp,v_y_sp);
        ...
        switch(stage)
            case 'trial_new_start'
                ...
                if rem(trial_success,cfg.numrepsp*numblock_sp)~=0
                    x_sp=blocksp_x(rem(trial_success,numblock_sp*cfg.numrepsp));
                    y_sp=blocksp_y(rem(trial_success,numblock_sp*cfg.numrepsp));
                else
                    x_sp=blocksp_x(end);
                    y_sp=blocksp_y(end);
                end
...
%                     disp(status)
            ...
            case 'put_on_fp'
                ...
                elseif ~change
                    ...
                    boxsp1= round(center(1)+x_sp-(window_fix/2));
                    boxsp2=round(center(2)-y_sp-(window_fix/2));
                    boxsp3=round(center(1)+x_sp+(window_fix/2));
                    boxsp4=round(center(2)-y_sp+(window_fix/2));
                end
...
%                     disp(status)
                ...
                elseif GetSecs-t_start_trial>=cfg.t_waitfixation_fp
...
%                     disp(status)
                ...
                if not((x_eye >= (box1))&&(x_eye <= (box3))&&(y_eye >= (box2))&&(y_eye <= (box4)))
                    ...
                    stage='inter_trial_interval';
                elseif GetSecs-t_start_trial>=cfg.t_fixation_fp
                    Results.HoldCompFP(trial_total)=GetSecs;
                    Eyelink('Message','HoldCompFP');
                    disp('HoldCompFP')
                    stage='put_on_sp';
                end
                % check if key is pressed
...
%                     disp(status)
                ...
                Screen('DrawingFinished',window);
                t_start_trial=Screen('Flip',window);
                Results.PutOnSP(trial_total)=t_start_trial;
...
%                     disp(status)
                ...
                if not((x_eye >= (box1))&&(x_eye <= (box3))&&(y_eye >= (box2))&&(y_eye <= (box4)))
                    ...
                    stage='inter_trial_interval';
                elseif GetSecs-t_start_trial>=cfg.t_fixation_fp_sp
...
disp('KeepHoldCompFP')
            ...
            case 'put_off_fp'
                ...
                % draw saccade target, update time

                if flag==8
%                     status= Eyelink('IsConnected');
%                     disp(status)
                    disp('put_off_fp')
                    Eyelink('Message', 'PutOffFP');
                ...
                t_start_trial=Screen('Flip',window);
                Results.PutOffFP(trial_total)=t_start_trial;
                % draw box on eyelink machine, representing window of
                % accepted eye position
                Eyelink('command','clear_screen %d', 0);
...
disp('FixInSP')
                ...
                elseif GetSecs-t_start_trial>=cfg.t_waitfixation_sp
                    stage='inter_trial_interval';
                end
                if Flash
                    % draw background, update time
...
%                     disp(status)
                ...
                if not((x_eye >= (boxsp1))&&(x_eye <= (boxsp3))&&(y_eye >= (boxsp2))&&(y_eye <= (boxsp4)))
                    ...
                    stage='inter_trial_interval';
                elseif GetSecs-t_start_trial>=cfg.t_fixation_sp
                    Results.HoldCompFP(trial_total)=GetSecs;
                    Eyelink('Message', 'HoldCompFP');
disp('HoldCompFP')
                    stage='reward';
            ...
            case 'reward'
                ...
                end                % Eyelink message, reward and reward amount
                % update time



                %                 if rem(trial_success,cfg.numrep_sp*numblock_sp)==0
                %                     ramidx=randperm(size(allcomb_sp,1));
                %                     allcomb_sp=allcomb_sp(ramidx,:);
                %                     blocksp_x=repelem(allcomb_sp(:,1),cfg.numrepsp);
                %                     blocksp_y=repelem(allcomb_sp(:,2),cfg.numrepsp);
            ...
            case 'inter_trial_interval'
                ...
                % update time
                t_start_trial=Screen('Flip',window);
                Results.ITI(trial_total)=t_start_trial;
                Eyelink('Message','ITI');
