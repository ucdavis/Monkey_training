...
try
    ...
    % Set pathway 
    path = strcat('D:\EyelinkData\','MGSaccadeDiffDir\',cfg.sub,'\',currDate,'\'); % where to keep the edf files
...
x_fp=cfg.fp_x*ppd;
    ...
    elseif cfg.polar
        ...
        v_y_sp=cfg.radius*ppd*sin(cfg.degree);
         v_x_sp2=cfg.radius*ppd*cos(cfg.degree2);
        v_y_sp2=cfg.radius*ppd*sin(cfg.degree2);
         v_x_sp3=cfg.radius*ppd*cos(cfg.degree3);
        v_y_sp3=cfg.radius*ppd*sin(cfg.degree3);
    end
    ...
    if ~cfg.polar
        ...
        allcomb_sp1=reshape(c_sp,[],2);
        ramidx_sp=randperm(size(allcomb_sp1,1));
        allcomb_sp1=allcomb_sp1(ramidx_sp,:);
        numblock_sp=size(allcomb_sp1,1);
    elseif cfg.polar
        %polar saccade point all combination
        allcomb_sp1=[cfg.fp_x*ppd+v_x_sp',cfg.fp_y*ppd+v_y_sp'];
        ramidx=randperm(size(allcomb_sp1,1));
        allcomb_sp1=allcomb_sp1(ramidx,:);
        numblock_sp1=size(allcomb_sp1,1);


 allcomb_sp2=repmat([cfg.fp_x*ppd+v_x_sp2',cfg.fp_y*ppd+v_y_sp2'],2,1);
        ramidx=randperm(size(allcomb_sp2,1));
        allcomb_sp2=allcomb_sp2(ramidx,:);
        numblock_sp=size(allcomb_sp1,1);


 allcomb_sp3=repmat([cfg.fp_x*ppd+v_x_sp3',cfg.fp_y*ppd+v_y_sp3'],4,1);
        ramidx=randperm(size(allcomb_sp3,1));
        allcomb_sp3=allcomb_sp3(ramidx,:);
        numblock_sp=size(allcomb_sp1,1);
    end
    for i=1:size(allcomb_sp1,1)
        b1= round(center(1)+allcomb_sp1(i,1)-(window_sp_fix/2));
        b2=round(center(2)-allcomb_sp1(i,2)-(window_sp_fix/2));
        b3=round(center(1)+allcomb_sp1(i,1)+(window_sp_fix/2));
        b4=round(center(2)-allcomb_sp1(i,2)+(window_sp_fix/2));
  ...
  box1p= round(center(1)+x_fp-(window_fix/2));
    ...
    blocksp_x1=repelem(allcomb_sp1(:,1),cfg.numrepsp);
    blocksp_y1=repelem(allcomb_sp1(:,2),cfg.numrepsp);
 blocksp_x2=repelem(allcomb_sp2(:,1),cfg.numrepsp);
    blocksp_y2=repelem(allcomb_sp2(:,2),cfg.numrepsp);
 blocksp_x3=repelem(allcomb_sp3(:,1),cfg.numrepsp);
    blocksp_y3=repelem(allcomb_sp3(:,2),cfg.numrepsp);
  ramidx_sp=randperm(3);
                    blocksp_xt=[blocksp_x1 blocksp_x2 blocksp_x3];
                    blocksp_xt=blocksp_xt(:,ramidx_sp);

                    blocksp_x=reshape(blocksp_xt,[],1);
                     blocksp_yt=[blocksp_y1 blocksp_y2 blocksp_y3];
                                                             blocksp_yt=blocksp_yt(:,ramidx_sp);

                    blocksp_y=reshape(blocksp_yt,[],1);
    ...
    while trial_success>0
        switch(stage)
            case 'trial_new_start'
                ...
                if rem(trial_success+skip_trial,cfg.numrepsp*numblock_sp*3)~=0
                    x_sp=blocksp_x(rem(trial_success+skip_trial,numblock_sp*cfg.numrepsp*3));
                    y_sp=blocksp_y(rem(trial_success+skip_trial,numblock_sp*cfg.numrepsp*3));
                else
                    x_sp=blocksp_x(end);
                    y_sp=blocksp_y(end);
                end
            ...
            case 'reward'
                ...
                if rem(trial_success+skip_trial,cfg.numrepsp*numblock_sp*3)==0
                    ...
                    blocksp_y1=repelem(allcomb_sp1(:,2),cfg.numrepsp);
                      ...
                      ramidx_sp=randperm(size(allcomb_sp2,1));
                    allcomb_sp2=allcomb_sp2(ramidx_sp,:);
                    blocksp_x2=repelem(allcomb_sp2(:,1),cfg.numrepsp);
                    blocksp_y2=repelem(allcomb_sp2(:,2),cfg.numrepsp);

                      ramidx_sp=randperm(size(allcomb_sp1,1));
                    allcomb_sp3=allcomb_sp3(ramidx_sp,:);
                    blocksp_x3=repelem(allcomb_sp3(:,1),cfg.numrepsp);
                    blocksp_y3=repelem(allcomb_sp3(:,2),cfg.numrepsp);
