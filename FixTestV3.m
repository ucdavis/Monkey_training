...
try
    ...
    window_fix=round(15*ppd); % the size of the accepted window for fixation point

    t_waitforfixation=2.0; % wait time for subjuct to fix when fp is first presented
    t_fixation=0.1;% time required to hold fixation (s)
    ...
    Eyelink('Command','screen_phys_coords = -343.0 193 343.0 -193 '); % this is the size of the default screen Dell 2005FPW
    % distance in mm from the center of the screen to [left top right bottom]
    % edge of screen
    Eyelink('Command', 'screen_distance = 78') %  Need to get from experimenter distance from center of eye to the center of the screen
    ...
    Eyelink('Command', 'calibration_type = HV3'); % horizontal-vertical 9-points
